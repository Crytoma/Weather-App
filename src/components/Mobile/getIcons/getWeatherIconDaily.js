...
export function getWeatherIcon(weather,day) {
    ...
    const ID = weather['daily'][day]['weather'][0]['id'];
    const clouds = weather['daily'][day]['clouds'];
    const isPartlyCloudy = clouds <= 25;
    const isScatteredClouds = clouds <= 50;
    const sunrise = weather['daily'][day]["sunrise"];
    const sunset = weather['daily'][day]["sunset"];
    const now = weather['daily'][day]['dt'];
    ...
    //the 2D array:
    var weather_type = (ID/100).toPrecision(1);
